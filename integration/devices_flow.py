...
import integration.devices_data_model as dm
from integration.devices_data_source import DataSource
...
def get_data_from_source():
    ...
    try:
        devices_api = DataSource(           
            token=cfg.SRC_TOKEN,
            url_devices=cfg.SRC_URL_GET_DEVICES,
            url_users=cfg.SRC_URL_GET_USERS,
            url_logins_vs_visits=cfg.SRC_URL_GET_LOGINS_VS_VISITS,
            url_general_statistics=cfg.SRC_URL_GET_GENERAL_STATISTICS

            
        )

        return devices_api.get_df_merge_data()
...
if __name__ == '__main__':
    ...
    smTaskUtil = sm.SmTaskUtil(reference_id=args.reference_id, task_name=cfg.TASK1_DEVICES_CODE, test_mode=False)
